...
const ellipseGalleryData: Gallary[] = [
  {
...
export class DataResolver implements Resolve<Gallary[]>  {
  constructor(
    public overlay: GlobalOverlayService,
  ) {}

  resolve(): Observable<Gallary[]> {
    return of(ellipseGalleryData).pipe(delay(2000), tap(() => this.overlay.hide()));
  }
}
